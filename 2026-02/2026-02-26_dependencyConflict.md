...
GitHub 上で PR を create したところ、GitHub Actions の CI で以下のジョブが失敗した。
...
複数の開発者が同じリポジトリに頻繁にコードを統合（＝マージ）するためのプロセスのこと。

#### 実際の手順

1. 開発者がコードをGitなどのバージョン管理システムにプッシュする
2. 自動でテストスクリプトや静的解析ツールが走る
3. テストにパスすればマージ可能、失敗すれば修正を促す
...
👉 *CIのエラーメッセージは「直接の原因」ではなく「結果」であることがある （ログを追い、依存関係まで掘る必要がある）*
...
📝 **Brakeman** ：
Brakeman は Rails アプリを静的解析し、潜在的な「セキュリティリスク」や「依存関係の問題」を検出するツール
...
- `Rails 7.2.3` は `minitest >= 5.1` を依存指定しているが、`minitest 6.0.2` では一部 API が変更されており、結果としてテストランナーがエラーになった。*

  👉 *Rubyの問題ではなく、`minitest` のバージョンを修正して対応することとする*

### scan_ruby failure の原因

- `Ruby 3.2.3` は安定版だが、 (「2026-03-31 に EOL 予定」であるため、`Brakeman` がそれを `Warning` として検出してしまう。*（`exit code 3` で CI を落とす仕様）*

  👉 *本件は「セキュリティ脆弱性」を示すものではなく、「依存関係」の問題*
