...
## ローカルでの準備
  ...
  - `master.key` は、Rails の秘密情報（DB パスワード、API キー等）を守るためのもの <br>🤛 *通常は`.gitignore` に含めてセキュリティを守り、GitHub には `credentials.yml.enc` だけを置く（金庫`enc`は公開可、鍵`master.key`は絶対に非公開）*
  ...
  ### `rails credentials:edit` がやっていること
    ...
    - 内容が同じなら Git 上では差分が出ない 🤛 *`git status`には上がらない*
